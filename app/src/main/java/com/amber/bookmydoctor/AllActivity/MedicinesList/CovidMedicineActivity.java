...
public class CovidMedicineActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_covid_medicine);

        // Initialize Firebase
        databaseReference = FirebaseDatabase.getInstance().getReference("CovidMedicines");
        ...
        uploadButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                // Create an instance of CovidModel
                CovidModel covidModel = new CovidModel("", medicineName, medicinePrice, companyName, manufacturingYear);

                // Save data to Firebase Realtime Database
                saveToFirebase(covidModel);
            }
        });
    }
        ...
        if (requestCode == REQUEST_IMAGE_CAPTURE && resultCode == RESULT_OK && data != null) {
            ...
            if (extras != null) {
                ...
                if (image instanceof Bitmap) {
                    ...
                    Log.d("CovidMedicineActivity", "Image URI: " + imageUri);
                }
            }
        } else {
            // Log the requestCode and resultCode for debugging
            Log.d("CovidMedicineActivity", "Request Code: " + requestCode + ", Result Code: " + resultCode);
    ...
    private void saveToFirebase(CovidModel covidModel) {
        // Check if all required details are filled
        if (isDetailsFilled(covidModel)) {
            if (imageUri != null) {
                // Generate a unique key for the medicine entry
                String key = databaseReference.push().getKey();

                // Save the data to the database, including the image URL
                covidModel.setImageUrl(imageUri.toString());
                databaseReference.child(key).setValue(covidModel);
    ...
    // Check if all required details are filled
    private boolean isDetailsFilled(CovidModel covidModel) {
        return !covidModel.getMedicineName().isEmpty() &&
                !covidModel.getMedicinePrice().isEmpty() &&
                !covidModel.getCompanyName().isEmpty();
    }
