...
public class SkinMedicineActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_skin_medicine);

        // Initialize Firebase
        databaseReference = FirebaseDatabase.getInstance().getReference("SkinMedicines");
        ...
        uploadButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                // Create an instance of SkinModel
                SkinModel skinModel = new SkinModel("", medicineName, medicinePrice, companyName, manufacturingYear);

                // Save data to Firebase Realtime Database
                saveToFirebase(skinModel);
            }
        });
    }
        ...
        if (requestCode == REQUEST_IMAGE_CAPTURE && resultCode == RESULT_OK && data != null) {
            ...
            if (extras != null) {
                ...
                if (image instanceof Bitmap) {
                    ...
                    Log.d("SkinMedicineActivity", "Image URI: " + imageUri);
                }
            }
        } else {
            // Log the requestCode and resultCode for debugging
            Log.d("SkinMedicineActivity", "Request Code: " + requestCode + ", Result Code: " + resultCode);
    ...
    private void saveToFirebase(SkinModel skinModel) {
        // Check if all required details are filled
        if (isDetailsFilled(skinModel)) {
            if (imageUri != null) {
                // Generate a unique key for the medicine entry
                String key = databaseReference.push().getKey();

                // Save the data to the database, including the image URL
                skinModel.setImageUrl(imageUri.toString());
                databaseReference.child(key).setValue(skinModel);
    ...
    // Check if all required details are filled
    private boolean isDetailsFilled(SkinModel skinModel) {
        return !skinModel.getMedicineName().isEmpty() &&
                !skinModel.getMedicinePrice().isEmpty() &&
                !skinModel.getCompanyName().isEmpty();
    }
}
